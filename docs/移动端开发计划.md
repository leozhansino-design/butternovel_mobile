# ButterNovel 移动端 App 开发计划

> 短篇小说阅读与创作平台 - Google Play & App Store

**版本**: 4.0
**更新日期**: 2025-12-23
**目标**: 打造抖音式短篇小说体验

---

## 目录

1. [产品定位](#产品定位)
2. [核心功能](#核心功能)
3. [技术方案](#技术方案)
4. [代码复用分析](#代码复用分析)
5. [开发阶段详解](#开发阶段详解)
6. [推送通知系统](#推送通知系统)
7. [测试策略](#测试策略)
8. [API 设计](#api-设计)
9. [上架准备](#上架准备)

---

## 产品定位

### 目标用户

- 喜欢碎片化阅读的年轻用户
- 想要分享自己故事的创作者
- 全球英语用户群体

### 核心特点

| 特点 | 说明 |
|------|------|
| 只做短篇 | 5,000-20,000 字的完整故事 |
| 抖音式推荐 | For You 垂直滑动浏览 |
| 人人可创作 | 一个账号 = 读者 + 作者 |
| 社交属性 | 关注、点赞、评论、分享 |

### 与 Web 端差异

| Web 端 | 移动端 |
|--------|--------|
| 长篇 + 短篇 | 只做短篇 |
| 章节制 | 完整故事 |
| 管理员上传 | 用户自主创作 |
| 传统列表浏览 | 抖音式滑动推荐 |

---

## 核心功能

### 底部导航 (5 Tabs)

```
┌─────────┬─────────┬─────────┬─────────┬─────────┐
│ For You │Following│   ➕    │Bookshelf│ Profile │
└─────────┴─────────┴─────────┴─────────┴─────────┘
```

### Tab 1: For You (推荐)

**交互设计**:
- 全屏卡片式浏览
- 上滑下一个，下滑上一个
- 右侧操作栏（❤️ 💬 📚 ↗️）
- 点击卡片进入阅读器
- 顶部搜索入口
- 下拉刷新

**推荐算法**:
```
用户兴趣标签     30%
阅读历史相似     25%
点赞收藏相关     20%
热门趋势         15%
新发布内容       10%
```

### Tab 2: Following (关注)

**功能**:
- 关注作者的最新发布
- 时间线形式展示
- 未读红点提醒
- 点击直接进入阅读

### Tab 3: Create (创作) ➕

**功能**:
- 创建新短篇
- 管理已创作作品
- 草稿箱
- 作品数据统计

**创作流程**:
```
1. 填写标题
2. 选择分类/标签
3. 上传封面（可选）
4. 编写内容
5. 预览
6. 发布/存草稿
```

### Tab 4: Bookshelf (书架)

**功能**:
- 收藏的故事
- 阅读历史
- 阅读进度
- 继续阅读

### Tab 5: Profile (我的)

**功能**:
- 个人资料编辑
- 阅读统计
- 创作统计
- 设置入口
- 通知中心
- 登出

### 阅读器

**短篇专用设计**:
```
┌─────────────────────────────────┐
│ ← Back          Story Title    │
├─────────────────────────────────┤
│                                 │
│     [沉浸式阅读内容区域]         │
│                                 │
├─────────────────────────────────┤
│ Aa   🌙   📖   💬              │
│ 字体  夜间  进度  评论          │
└─────────────────────────────────┘
```

### 登录注册

**支持方式**:
- Google 登录 ⭐
- Apple 登录 ⭐ (iOS 必须)
- Email + Password
- 游客模式（有限功能）

---

## 技术方案

### 技术栈

```json
{
  "框架": "Expo SDK 52+",
  "核心": "React Native 0.76+",
  "路由": "Expo Router",
  "状态": "@tanstack/react-query + zustand",
  "表单": "react-hook-form + zod",
  "UI": "NativeWind (Tailwind CSS)",
  "存储": "expo-secure-store + async-storage",
  "认证": "expo-auth-session",
  "推送": "expo-notifications + FCM/APNs",
  "图片": "expo-image + expo-image-picker"
}
```

### 项目结构

```
mobile/
├── app/                      # Expo Router 页面
│   ├── _layout.tsx           # 根布局
│   ├── (tabs)/               # Tab 导航
│   ├── auth/                 # 认证页面
│   ├── reader/               # 阅读器
│   ├── story/                # 故事详情
│   ├── author/               # 作者主页
│   ├── create/               # 创作页面
│   └── settings/             # 设置页面
│
├── components/
│   ├── ui/                   # 基础 UI 组件
│   ├── story/                # 故事相关组件
│   ├── reader/               # 阅读器组件
│   └── create/               # 创作组件
│
├── lib/
│   ├── api/                  # API 客户端
│   ├── utils/                # 工具函数 (可从 web 复用)
│   └── validators/           # 验证逻辑 (可从 web 复用)
│
├── hooks/                    # 自定义 Hooks
├── stores/                   # Zustand 状态
├── services/                 # 推送等服务
├── assets/                   # 图片/字体
└── __tests__/                # 测试文件
```

---

## 代码复用分析

### ✅ 可直接复用 (从 web/src/lib 复制)

| 文件 | 说明 | 复用方式 |
|------|------|----------|
| `format.ts` | 数字格式化 (1.5k, 2.3m) | 直接复制 |
| `validators.ts` | Zod 验证逻辑 | 复制，移除图片验证部分 |
| `constants.ts` | 分类、状态常量 | 直接复制 |
| `utils.ts` 部分 | `generateSlug`, `formatDate`, `truncate` | 直接复制 |

**format.ts 复用示例**:
```typescript
// 可直接复制到 mobile/lib/utils/format.ts
export function formatNumber(num: number): string {
  const rounded = Math.floor(Math.abs(num))
  if (rounded < 1000) return rounded.toString()
  if (rounded < 1000000) return `${(rounded / 1000).toFixed(1)}k`
  return `${(rounded / 1000000).toFixed(1)}m`
}
```

**validators.ts 部分复用**:
```typescript
// 可复用的 Schema (复制到 mobile/lib/validators.ts)
export const ratingSchema = z.object({
  score: z.coerce.number().int().refine(
    (val) => [2, 4, 6, 8, 10].includes(val),
    { message: 'Rating must be one of: 2, 4, 6, 8, 10' }
  ),
  review: z.string().max(1000).optional(),
})

export const registerSchema = z.object({
  email: z.string().email('Invalid email format'),
  password: z.string().min(6).max(50),
  name: z.string().min(1).max(50).optional(),
})

export const profileUpdateSchema = z.object({
  name: z.string().min(1).max(50).optional(),
  bio: z.string().max(500).optional(),
})
```

### ⚠️ 需要修改后复用

| 文件 | 说明 | 修改点 |
|------|------|--------|
| `validators.ts` | 图片验证 | 移除 `window.Image`，用 expo-image-manipulator |
| API 类型定义 | TypeScript 接口 | 直接复用，但需创建对应文件 |

### ❌ 必须重写

| 功能 | 原因 |
|------|------|
| 所有 UI 组件 | React Native 使用 View/Text，不是 div/span |
| 样式系统 | Tailwind → NativeWind (语法略有不同) |
| 导航系统 | Next.js Router → Expo Router |
| 存储逻辑 | localStorage → AsyncStorage/SecureStore |
| 认证流程 | NextAuth → expo-auth-session |
| 图片处理 | next/image → expo-image |
| 推送通知 | 无 → expo-notifications (新功能) |

### 类型定义复用

从 web 项目提取并复用的类型定义：

```typescript
// mobile/lib/types.ts

// 用户类型
export interface User {
  id: string
  email: string
  name: string | null
  avatar: string | null
  bio: string | null
}

// 短篇小说类型
export interface ShortNovel {
  id: number
  title: string
  blurb: string
  shortNovelGenre: string | null
  wordCount: number
  viewCount: number
  likeCount: number
  averageRating: number | null
  totalRatings: number
  authorId: string
  authorName: string
  isShortNovel: boolean
  createdAt: string
  chapters?: Chapter[]
}

// 章节类型
export interface Chapter {
  id: number
  title: string
  content: string
  chapterNumber: number
  wordCount: number
}

// 评分类型
export interface Rating {
  id: string
  score: number // 2, 4, 6, 8, 10
  review: string | null
  likeCount: number
  userId: string
  user: {
    id: string
    name: string | null
    avatar: string | null
  }
  createdAt: string
}

// 段落评论类型
export interface ParagraphComment {
  id: string
  content: string
  paragraphIndex: number
  likeCount: number
  replyCount: number
  userId: string
  user: {
    id: string
    name: string | null
    avatar: string | null
    level: number
  }
  createdAt: string
}

// 通知类型
export interface Notification {
  id: string
  type: 'NEW_FOLLOWER' | 'NEW_LIKE' | 'NEW_COMMENT' | 'NEW_STORY'
  title: string
  content: string
  isRead: boolean
  data: Record<string, any>
  createdAt: string
}
```

---

## 开发阶段详解

### Phase 1: 项目搭建与基础架构

**开发顺序**:
```
1. 创建 Expo 项目
   └── npx create-expo-app@latest mobile --template blank-typescript

2. 配置基础依赖
   ├── 安装 Expo Router
   ├── 安装 NativeWind
   ├── 安装状态管理 (zustand + react-query)
   └── 配置 TypeScript

3. 搭建导航架构
   ├── 创建 app/_layout.tsx (根布局)
   ├── 创建 app/(tabs)/_layout.tsx (Tab导航)
   └── 创建 5 个 Tab 页面占位

4. 封装 API 客户端
   ├── 创建 lib/api/client.ts
   ├── 实现请求拦截器
   ├── 实现 Token 管理
   └── 创建基础 API 方法 (get/post/put/delete)

5. 复制可复用代码
   ├── 从 web 复制 format.ts
   ├── 从 web 复制 constants.ts
   ├── 从 web 复制 validators.ts (移除浏览器相关)
   └── 创建类型定义文件
```

**需要测试** 🧪:
- [ ] API 客户端基础请求
- [ ] 导航跳转正常
- [ ] NativeWind 样式生效

---

### Phase 2: 认证系统

**开发顺序**:
```
1. 创建认证状态管理
   └── stores/authStore.ts (zustand)

2. 实现 Token 存储
   ├── 使用 expo-secure-store 存储 token
   └── 实现 token 持久化

3. 实现邮箱登录/注册
   ├── 创建 app/auth/login.tsx
   ├── 创建 app/auth/register.tsx
   ├── 使用 react-hook-form + zod 验证
   └── 调用现有 API: POST /api/auth/register

4. 实现 Google 登录
   ├── 配置 expo-auth-session
   ├── 配置 Google Cloud Console
   └── 实现 OAuth 流程

5. 实现 Apple 登录 (iOS 必须)
   ├── 配置 expo-apple-authentication
   ├── 配置 Apple Developer Console
   └── 实现 Sign in with Apple

6. 实现登出功能
   └── 清除 token + 重置状态

7. 实现游客模式
   ├── 允许浏览但限制操作
   └── 显示登录提示
```

**需要测试** 🧪:
- [ ] 邮箱注册流程
- [ ] 邮箱登录流程
- [ ] Google 登录 (需要真机测试)
- [ ] Apple 登录 (需要 iOS 真机测试)
- [ ] Token 持久化 (关闭App后重开)
- [ ] 登出后状态清理
- [ ] 游客模式限制

---

### Phase 3: For You 推荐页

**开发顺序**:
```
1. 创建故事卡片组件
   ├── components/story/StoryCard.tsx
   ├── 显示标题、作者、预览
   ├── 显示评分、阅读时间
   └── 右侧操作栏 (点赞/评论/收藏/分享)

2. 实现垂直滑动浏览
   ├── 使用 FlatList + pagingEnabled
   ├── 实现无限滚动加载
   └── 实现下拉刷新

3. 实现点赞功能
   ├── 调用 API: POST /api/shorts/[id]/recommend
   ├── 乐观更新 UI
   └── 登录检查

4. 实现收藏功能
   ├── 调用 API: POST /api/library
   ├── 乐观更新 UI
   └── 登录检查

5. 实现分享功能
   └── 使用 expo-sharing

6. 实现搜索入口
   ├── 创建 app/search.tsx
   ├── 调用 API: GET /api/search
   └── 搜索建议: GET /api/search/suggestions
```

**需要测试** 🧪:
- [ ] 故事列表加载
- [ ] 滑动切换流畅度
- [ ] 无限滚动加载
- [ ] 下拉刷新
- [ ] 点赞交互 (登录/未登录)
- [ ] 收藏交互 (登录/未登录)
- [ ] 分享功能
- [ ] 搜索功能

---

### Phase 4: 阅读器

**开发顺序**:
```
1. 创建阅读器页面
   └── app/reader/[id].tsx

2. 创建阅读设置状态
   └── stores/readerStore.ts (持久化)

3. 实现阅读内容显示
   ├── 调用 API: GET /api/shorts/[id]
   └── 渲染故事内容

4. 实现阅读设置
   ├── components/reader/ReaderSettings.tsx
   ├── 字体大小调节
   ├── 背景颜色切换 (白/米/灰/黑)
   ├── 行间距调节
   └── 设置持久化到 AsyncStorage

5. 实现段落评论
   ├── 每个段落末尾显示评论按钮
   ├── 调用 API: GET /api/paragraph-comments
   ├── 调用 API: POST /api/paragraph-comments
   └── components/reader/ParagraphComments.tsx

6. 实现评分功能
   ├── 底部菜单评分入口
   ├── 阅读结束评分弹窗
   ├── 调用 API: POST /api/novels/[id]/rate
   └── components/reader/RatingModal.tsx

7. 实现阅读进度保存
   ├── 保存滚动位置到本地
   └── 下次打开恢复位置
```

**需要测试** 🧪:
- [ ] 内容正确加载
- [ ] 滚动阅读流畅
- [ ] 字体大小调节生效
- [ ] 背景颜色切换生效
- [ ] 设置持久化
- [ ] 段落评论加载
- [ ] 发表段落评论
- [ ] 评分提交
- [ ] 重复评分提示
- [ ] 阅读进度保存/恢复

---

### Phase 5: 创作功能

**开发顺序**:
```
1. 创建我的作品页面
   └── app/(tabs)/create.tsx

2. 实现作品列表
   ├── 调用 API: GET /api/mobile/my-stories
   ├── 显示草稿/已发布状态
   └── 显示阅读量/点赞数

3. 创建新建故事页面
   └── app/create/new.tsx

4. 实现标题输入
   ├── 使用 react-hook-form
   └── Zod 验证 (复用 web 的 schema)

5. 实现分类选择
   ├── 从 constants.ts 读取分类列表
   └── components/create/CategoryPicker.tsx

6. 实现内容编辑器
   ├── components/create/Editor.tsx
   ├── 富文本编辑 (加粗/斜体)
   ├── 实时字数统计
   └── 自动保存草稿 (本地)

7. 实现封面上传 (可选)
   ├── 使用 expo-image-picker
   ├── 图片压缩
   └── 上传到服务器

8. 实现发布/存草稿
   ├── 调用 API: POST /api/mobile/stories
   └── 发布成功跳转

9. 实现编辑已有作品
   └── app/create/[id]/edit.tsx
```

**需要测试** 🧪:
- [ ] 我的作品列表加载
- [ ] 新建故事流程完整
- [ ] 标题验证
- [ ] 分类选择
- [ ] 内容编辑器功能
- [ ] 字数统计准确
- [ ] 草稿自动保存
- [ ] 封面上传
- [ ] 发布成功
- [ ] 编辑已有作品

---

### Phase 6: 关注系统

**开发顺序**:
```
1. 创建 Following 页面
   └── app/(tabs)/following.tsx

2. 实现关注动态列表
   ├── 调用 API: GET /api/mobile/following-updates
   └── 时间线形式展示

3. 创建作者主页
   ├── app/author/[id].tsx
   ├── 显示作者信息
   ├── 显示作者作品列表
   └── 关注/取关按钮

4. 实现关注/取关
   ├── 调用 API: POST /api/user/follow
   ├── 调用 API: DELETE /api/user/follow
   └── 乐观更新 UI

5. 实现未读标记
   └── 根据最后查看时间计算
```

**需要测试** 🧪:
- [ ] 关注动态列表加载
- [ ] 作者主页显示
- [ ] 关注功能
- [ ] 取消关注功能
- [ ] 未读标记显示

---

### Phase 7: 书架与个人中心

**开发顺序**:
```
1. 创建书架页面
   └── app/(tabs)/bookshelf.tsx

2. 实现收藏列表
   ├── 调用 API: GET /api/library
   └── 支持移除收藏

3. 实现阅读历史
   ├── 调用 API: GET /api/reading-history
   └── 显示阅读进度

4. 创建个人中心页面
   └── app/(tabs)/profile.tsx

5. 实现个人资料编辑
   ├── app/settings/profile.tsx
   ├── 调用 API: PUT /api/profile
   └── 头像上传: POST /api/profile/avatar

6. 实现阅读/创作统计
   └── 调用 API: GET /api/profile/stats

7. 创建设置页面
   ├── app/settings/index.tsx
   ├── 通知设置入口
   └── 账号设置
```

**需要测试** 🧪:
- [ ] 收藏列表加载
- [ ] 移除收藏
- [ ] 阅读历史显示
- [ ] 个人资料编辑
- [ ] 头像上传
- [ ] 统计数据显示

---

### Phase 8: 推送通知系统

**开发顺序**:
```
1. 配置推送权限
   ├── 安装 expo-notifications
   └── 安装 expo-device

2. 获取推送 Token
   ├── iOS: APNs Token
   └── Android: FCM Token

3. 上传 Token 到服务器
   └── 调用 API: POST /api/mobile/push-token

4. 创建通知服务
   └── services/notification.ts

5. 实现前台通知处理
   └── App 内显示通知

6. 实现后台通知处理
   └── 点击通知跳转

7. 创建通知中心页面
   ├── app/notifications.tsx
   ├── 调用 API: GET /api/notifications
   └── 标记已读

8. 实现通知设置
   ├── app/settings/notifications.tsx
   └── 调用 API: PUT /api/notifications/preferences
```

**需要测试** 🧪:
- [ ] 推送权限请求
- [ ] Token 获取 (需真机)
- [ ] 前台收到通知
- [ ] 后台收到通知
- [ ] 点击通知跳转正确
- [ ] 通知中心列表
- [ ] 标记已读
- [ ] 通知设置生效

---

### Phase 9: 优化与上架

**开发顺序**:
```
1. 性能优化
   ├── 图片懒加载优化
   ├── 列表虚拟化
   ├── 减少重渲染
   └── 网络请求缓存

2. 错误处理
   ├── 全局错误边界
   ├── 网络错误提示
   └── 离线模式支持

3. 准备上架资源
   ├── App 图标 (多尺寸)
   ├── 启动画面
   ├── 商店截图
   ├── 隐私政策页面
   └── 应用描述

4. 构建测试版本
   ├── eas build --profile preview
   └── 内部测试分发

5. 修复测试反馈

6. 构建生产版本
   └── eas build --profile production

7. 提交审核
   ├── Google Play Console
   └── App Store Connect
```

---

## 推送通知系统

### 推送类型

| 类型 | 触发条件 | 优先级 |
|------|----------|--------|
| NEW_STORY | 关注的作者发布新作品 | 高 |
| NEW_COMMENT | 收到评论回复 | 高 |
| NEW_LIKE | 作品收到点赞 | 中 |
| NEW_FOLLOWER | 有新粉丝关注 | 中 |
| SYSTEM | 系统公告 | 低 |

### 服务端配置

需要在 web 后端添加以下 API：

```typescript
// POST /api/mobile/push-token
// 注册推送 Token
Body: {
  token: string,           // 推送 Token
  platform: 'ios' | 'android',
  deviceId: string         // 设备唯一标识
}

// PUT /api/notifications/preferences
// 更新通知偏好
Body: {
  newStory: boolean,       // 关注作者新作品
  newComment: boolean,     // 新评论
  newLike: boolean,        // 新点赞
  newFollower: boolean,    // 新粉丝
  system: boolean          // 系统通知
}
```

### 移动端实现

```typescript
// services/notification.ts
import * as Notifications from 'expo-notifications'
import * as Device from 'expo-device'
import { Platform } from 'react-native'
import { api } from '@/lib/api/client'

class NotificationService {
  async init() {
    // 1. 请求权限
    const { status } = await Notifications.requestPermissionsAsync()
    if (status !== 'granted') {
      console.log('Notification permission denied')
      return
    }

    // 2. 获取推送 Token
    if (Device.isDevice) {
      const token = await Notifications.getExpoPushTokenAsync({
        projectId: 'your-expo-project-id'
      })

      // 3. 上传到服务器
      await this.registerToken(token.data)
    }

    // 4. 配置前台通知处理
    Notifications.setNotificationHandler({
      handleNotification: async () => ({
        shouldShowAlert: true,
        shouldPlaySound: true,
        shouldSetBadge: true,
      }),
    })
  }

  async registerToken(token: string) {
    const deviceId = await this.getDeviceId()
    await api.post('/mobile/push-token', {
      token,
      platform: Platform.OS,
      deviceId,
    })
  }

  // 监听通知点击
  addNotificationResponseListener(
    handler: (notification: Notifications.NotificationResponse) => void
  ) {
    return Notifications.addNotificationResponseReceivedListener(handler)
  }

  private async getDeviceId(): Promise<string> {
    // 使用 expo-device 或其他方式获取设备唯一标识
    return Device.modelId || 'unknown'
  }
}

export const notificationService = new NotificationService()
```

### 通知点击跳转

```typescript
// 在 App 入口配置
useEffect(() => {
  const subscription = notificationService.addNotificationResponseListener(
    (response) => {
      const data = response.notification.request.content.data

      switch (data.type) {
        case 'NEW_STORY':
          router.push(`/reader/${data.storyId}`)
          break
        case 'NEW_COMMENT':
          router.push(`/reader/${data.storyId}?comment=${data.commentId}`)
          break
        case 'NEW_FOLLOWER':
          router.push(`/author/${data.userId}`)
          break
        default:
          router.push('/notifications')
      }
    }
  )

  return () => subscription.remove()
}, [])
```

---

## 测试策略

### 测试分类

| 类型 | 工具 | 说明 |
|------|------|------|
| 单元测试 | Jest | 测试工具函数、Hooks |
| 组件测试 | React Native Testing Library | 测试 UI 组件 |
| E2E 测试 | Detox | 端到端测试流程 |
| 手动测试 | Expo Go / 真机 | 真实设备测试 |

### 开发过程中的测试流程

#### 1. 本地开发测试

```bash
# 启动开发服务器
npx expo start

# 快速测试选项
# 按 i - iOS 模拟器 (需要 Mac + Xcode)
# 按 a - Android 模拟器 (需要 Android Studio)
# 按 w - Web 浏览器 (快速预览 UI)
# 扫码 - 用 Expo Go 在真机测试
```

#### 2. 功能完成后测试

每完成一个功能模块，按以下检查清单测试：

**认证模块测试清单**:
```
□ 注册页面能正常打开
□ 输入无效邮箱显示错误提示
□ 密码少于6位显示错误提示
□ 注册成功后跳转正确
□ 登录成功后 Token 正确保存
□ 关闭 App 重新打开仍保持登录
□ 登出后返回登录页
□ Google 登录流程完整 (真机)
□ Apple 登录流程完整 (iOS 真机)
```

**API 调用测试清单**:
```
□ 网络正常时请求成功
□ 网络断开时显示错误提示
□ Token 过期时自动跳转登录
□ 请求超时有合理提示
```

#### 3. 真机测试

```bash
# 构建开发版本安装到真机
eas build --profile development --platform ios
eas build --profile development --platform android

# 或使用 Expo Go
# 手机下载 Expo Go App，扫描终端二维码
```

**必须在真机测试的功能**:
- Google/Apple 登录
- 推送通知
- 相机/相册权限
- 手势操作流畅度
- 性能表现

#### 4. 单元测试示例

```typescript
// __tests__/lib/format.test.ts
import { formatNumber } from '@/lib/utils/format'

describe('formatNumber', () => {
  it('should return number as string for values under 1000', () => {
    expect(formatNumber(0)).toBe('0')
    expect(formatNumber(100)).toBe('100')
    expect(formatNumber(999)).toBe('999')
  })

  it('should format thousands with k suffix', () => {
    expect(formatNumber(1000)).toBe('1k')
    expect(formatNumber(1500)).toBe('1.5k')
    expect(formatNumber(15000)).toBe('15k')
  })

  it('should format millions with m suffix', () => {
    expect(formatNumber(1000000)).toBe('1m')
    expect(formatNumber(1500000)).toBe('1.5m')
  })
})
```

#### 5. 组件测试示例

```typescript
// __tests__/components/StoryCard.test.tsx
import { render, fireEvent } from '@testing-library/react-native'
import { StoryCard } from '@/components/story/StoryCard'

describe('StoryCard', () => {
  const mockStory = {
    id: 1,
    title: 'Test Story',
    blurb: 'Test blurb...',
    authorName: 'Test Author',
    likeCount: 100,
    commentCount: 50,
  }

  it('should render story title', () => {
    const { getByText } = render(<StoryCard story={mockStory} />)
    expect(getByText('Test Story')).toBeTruthy()
  })

  it('should call onPress when tapped', () => {
    const onPress = jest.fn()
    const { getByTestId } = render(
      <StoryCard story={mockStory} onPress={onPress} />
    )
    fireEvent.press(getByTestId('story-card'))
    expect(onPress).toHaveBeenCalled()
  })

  it('should format like count correctly', () => {
    const { getByText } = render(<StoryCard story={mockStory} />)
    expect(getByText('100')).toBeTruthy()
  })
})
```

### 测试命令

```bash
# 运行所有测试
npm test

# 运行特定测试文件
npm test -- format.test.ts

# 运行测试并生成覆盖率报告
npm test -- --coverage

# 监听模式 (开发时使用)
npm test -- --watch
```

---

## API 设计

### 移动端专用 API (需要新增)

```typescript
// For You 推荐
GET /api/mobile/for-you
Query: { page, limit, tags? }
Response: { stories: Story[], nextPage: number }

// 关注动态
GET /api/mobile/following-updates
Query: { page, limit }
Response: { updates: Update[], nextPage: number }

// 创建故事
POST /api/mobile/stories
Body: {
  title: string,
  content: string,
  coverImage?: string,
  tags: string[],
  categoryId: number,
  isDraft: boolean
}

// 获取我的作品
GET /api/mobile/my-stories
Query: { page, limit, status?: 'published' | 'draft' }

// 推送 Token
POST /api/mobile/push-token
Body: { token, platform, deviceId }
```

### 复用现有 API

```typescript
// 收藏
POST /api/library
DELETE /api/library

// 评分
POST /api/novels/:id/rate
GET /api/novels/:id/ratings

// 段落评论
GET /api/paragraph-comments
POST /api/paragraph-comments

// 关注
POST /api/user/follow
DELETE /api/user/follow

// 搜索
GET /api/search

// 通知
GET /api/notifications
POST /api/notifications/:id/read

// 个人资料
GET /api/profile
PUT /api/profile
```

---

## 上架准备

### Google Play

| 要求 | 说明 |
|------|------|
| 开发者账号 | $25 一次性费用 |
| 隐私政策 | 必须提供 URL |
| 内容分级 | 填写问卷 |
| 截图 | 至少 2 张 |
| 功能图片 | 1024 x 500 |
| 应用图标 | 512 x 512 |
| App Bundle | .aab 格式 |

### App Store

| 要求 | 说明 |
|------|------|
| 开发者账号 | $99/年 |
| Sign in with Apple | **必须实现** |
| 隐私政策 | 必须提供 URL |
| App Privacy | 数据收集声明 |
| 截图 | 多尺寸 (6.7", 6.5", 5.5") |
| 应用图标 | 1024 x 1024 无透明 |
| 年龄分级 | 内容问卷 |

### 审核注意事项

**常见拒绝原因**:
1. 缺少 Sign in with Apple (iOS)
2. 隐私政策不完整
3. 功能不完整或崩溃
4. 截图与实际不符
5. 缺少内容举报机制

---

**让短篇阅读触手可及，让创作人人可为**
