...
| 任务 | 优先级 | 说明 |
|------|--------|------|
| For You 全屏滑动 | 高 | 抖音式，一页一个故事，不是卡片列表 |
| 主题色改为蓝色 | 高 | 简约高大上，不是黄色 |
| 毛玻璃效果 | 中 | expo-blur 实现 |
| 简化图标 | 中 | 只用必要的 Ionicons |
| 移除 mock 假数据 | 高 | 连接真实 API |
...
// 半透明毛玻璃背景
<BlurView intensity={80} tint="light" className="...">
...
| 功能模块 | 测试内容 | 优先级 |
|----------|----------|--------|
| **认证系统** | login/logout/token刷新 | 🔴 高 |
| **API 客户端** | 请求拦截器/错误处理/重试 | 🔴 高 |
| **状态管理** | authStore/readerStore 状态变化 | 🔴 高 |
| **工具函数** | format.ts/validators.ts | 🟡 中 |
| **表单验证** | 登录/注册/创作表单 | 🟡 中 |
...
| 功能 | 测试要点 | 设备要求 |
|------|----------|----------|
| Google 登录 | OAuth 流程完整 | Android/iOS |
| Apple 登录 | Sign in with Apple | iOS 必须 |
| 推送通知 | 前台/后台/点击跳转 | 真机必须 |
| 全屏滑动 | 流畅度、边界处理 | 真机 |
| 毛玻璃效果 | 渲染性能 | 真机 |
| 手势操作 | 滑动、长按、双击 | 真机 |
...
│   │   └── ...
│   └── story/               # 故事相关组件
│       ├── FullScreenStory.tsx  # 全屏故事
│       └── ActionButtons.tsx    # 右侧操作按钮
├── lib/
...
{
  "框架": "Expo SDK 54",
  "核心": "React Native 0.81.5 + React 19.1",
  "路由": "Expo Router 6.0",
